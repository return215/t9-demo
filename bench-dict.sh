...
# get signatures for later input

_keys=$(java -cp bin predictive.Words2SigProto <<< "${_dargs}") 

# Proto

# echo "Sigs2WordsProto" >> $_resfile
# 
# for i in $(seq 1 $_count); do
#   (time java -cp bin predictive.Sigs2WordsProto <<< "${_keys}" > /dev/null;) &>> $_resfile
# done
# 
# echo >> $_resfile
...
echo "Sigs2WordsList" >> $_resfile

for i in $(seq 1 $_count); do
  (time java -cp bin predictive.Sigs2WordsList <<< "${_keys}" > /dev/null;) &>> $_resfile
done
...
echo "Sigs2WordsMap" >> $_resfile

for i in $(seq 1 $_count); do
  (time java -cp bin predictive.Sigs2WordsMap <<< "${_keys}" > /dev/null;) &>> $_resfile
done
